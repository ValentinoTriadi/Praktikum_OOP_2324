...
public class Pembunuh implements Kelas {
    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        ...
        int darahTarget = target.getHealth();
        if (darahTarget > 20){
            darahTarget = 20;
        }
        System.out.println(self.getName() + " menyerap health " + target.getName() + " sebesar " + darahTarget);
        target.terimaSerangan(darahTarget);
        self.addHealth(darahTarget);    
    }
